...
void adcStartup(void)
{
    ...
    writeSingleRegister(CLOCK_ADDRESS, (CLOCK_DEFAULT & ~CLOCK_OSR_MASK) | CLOCK_OSR_2048); // | 0x0020
...
bool readData(adc_channel_data *DataStruct)
{
    ...
    for (i = 0; i < bytesPerWord; i++)
    {
        DataStruct->channel[0].bytes[i] = spiSendReceiveByte(crcTx[i]);
    }
    //DataStruct->channel[0].full_value = signExtend(&dataRx[0]);
    ...
    for (i = 0; i < bytesPerWord; i++)
    {
        DataStruct->channel[1].bytes[i] = spiSendReceiveByte(0x00);
    }
    //DataStruct->channel[1].full_value = signExtend(&dataRx[0]);
    //crcWord = calculateCRC(&dataRx[0], bytesPerWord, crcWord);
    ...
    for (i = 0; i < bytesPerWord; i++)
    {
        DataStruct->channel[2].bytes[i] = spiSendReceiveByte(0x00);
    }
    //DataStruct->channel[2].full_value = signExtend(&dataRx[0]);
    //crcWord = calculateCRC(&dataRx[0], bytesPerWord, crcWord);
    ...
    for (i = 0; i < bytesPerWord; i++)
    {
        DataStruct->channel[3].bytes[i] = spiSendReceiveByte(0x00);
    }
    //DataStruct->channel[3].full_value = signExtend(&dataRx[0]);
    //crcWord = calculateCRC(&dataRx[0], bytesPerWord, crcWord);
    ...
    DataStruct->channel[4].full_value = signExtend(&dataRx[0]);
    //crcWord = calculateCRC(&dataRx[0], bytesPerWord, crcWord);
    ...
    DataStruct->channel[5].full_value = signExtend(&dataRx[0]);
    //crcWord = calculateCRC(&dataRx[0], bytesPerWord, crcWord);
    ...
    DataStruct->channel[6].full_value = signExtend(&dataRx[0]);
    //crcWord = calculateCRC(&dataRx[0], bytesPerWord, crcWord);
    ...
    DataStruct->channel[7].full_value = signExtend(&dataRx[0]);
    //crcWord = calculateCRC(&dataRx[0], bytesPerWord, crcWord);
...
void extract_low_24_bits(adc_channel_data *data) {
    for (int i = 0; i < CHANNEL_COUNT; i++) 
		{
				for(int j = 0; j < 3; j++)
				{
					adc_tx_data[adc_data_cnt] = data->channel[i].bytes[j];
					adc_data_cnt++;
				}
    }
}
